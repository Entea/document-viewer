...
* rotation: [0, 1, 2, 3] -> rotates by `rotation * 90` degrees

The document JSON data should have an image template string in the `resources.page.image` section. The template should include `{page}`, `{size}` and `{orientation}`.

  http://s3.documentcloud.org/documents/19864/pages/goldman-sachs-internal-emails-p{page}-{size}-{rotation}.gif
